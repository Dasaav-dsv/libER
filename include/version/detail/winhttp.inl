...
namespace liber {
    ...
    public:
        ...
        decltype(&::WinHttpQueryDataAvailable) WinHttpQueryDataAvailable = _placeholder;
        decltype(&::WinHttpReadData) WinHttpReadData = _placeholder;

        winhttp_exports() noexcept {
#define LIBER_GET_SET_PROC(NAME) { FARPROC proc = GetProcAddress(handle, #NAME); if (proc) this->NAME = reinterpret_cast<decltype(this->NAME)>(proc); }
...
#undef LIBER_GET_SET_PROC
    ...
    public:
        ...
        bool send() noexcept {
            return winhttp.WinHttpSendRequest(this->native_handle(), WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0);
        }
        ...
        bool read_available(Iter first, Iter last, DWORD& count) noexcept {
            ...
            if (d > std::numeric_limits<DWORD>::max())
                return false;
            return winhttp.WinHttpReadData(this->native_handle(), (LPVOID)(std::addressof(*first)), static_cast<DWORD>(d), &count);
        }

    private:
        friend class winhttp_connection;
        
        winhttp_request(const std::shared_ptr<winhttp_handle>& connection, const wchar_t* verb, const std::wstring& obj)
            : winhttp_handle(winhttp.WinHttpOpenRequest(connection->native_handle(), verb, obj.c_str(), NULL, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE)),
    ...
    public:
        ...
        [[nodiscard]] static winhttp_request request(const std::shared_ptr<winhttp_connection>& connection,
    ...
    private:
        ...
        winhttp_connection(std::shared_ptr<winhttp_handle> session, const wchar_t* server)
            : winhttp_handle(winhttp.WinHttpConnect(session->native_handle(), server, INTERNET_DEFAULT_HTTPS_PORT, 0)),
    ...
    class winhttp_session : public winhttp_handle {
        using winhttp_handle::winhttp_handle;
        winhttp_session() : winhttp_handle(winhttp.WinHttpOpen(L"UserAgent/1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, NULL)) {}
    public:
        ...
        [[nodiscard]] static std::shared_ptr<winhttp_connection> connect(const std::shared_ptr<winhttp_session>& session, const wchar_t* server) noexcept {
            return std::shared_ptr<winhttp_connection>{ new winhttp_connection(session, server) };
        }
    };

    // Request and attempt to download a file
    // through an established winhttp connection
    [[nodiscard]] static std::string get_file(const std::shared_ptr<winhttp_connection>& connection, const std::wstring& name) noexcept {
        // Try to create and send the request and receive back the file bytes
        winhttp_request request = winhttp_connection::request(connection, L"GET", name);
        ...
        while (true) {
            ...
            if (!request.read_available(out.end() - cb_available, out.end(), cb_read))
                return "";
            if (!cb_read) break;
        }
        return out;
